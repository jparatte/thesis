function [ Y, t, tmt ] = dr_lle( data, no_dims, k, eig_impl)
...
%
% Last modified by J. Paratte for scalability (2017)
    ...
    %knn_params = {};
    %knn_params.flann_params = flann_params(n, d, 'kdtree', 1);
    %knn_params.type = 'flann';
    %[neighborhood, dd] = knn_search(X, k, knn_params);

    neighborhood = data.nn(:,1:k);
    
    if 0
        indy = kron((1:n)', ones(k, 1));
        indx = reshape(neighborhood', k*n, 1);
        dist_values = reshape(dd', k*n, 1);
        distance = sparse(indx, indy, dist_values, n, n);
        ...
        neighborhood = transpose(neighborhood);
        X = transpose(double(data.raw)); 
    end
    ...
    if strcmp(eig_impl, 'JDQR')
        ...
        [mappedX, eigenvals] = jdqr(M + eps * speye(n), no_dims + 1, tol, options);
    else
        options.disp = 0;
        options.isreal = 1;
        options.issym = 1;
        [mappedX, eigenvals] = eigs(M + eps * speye(n), no_dims + 1, tol, options);          % only need bottom (no_dims + 1) eigenvectors
    end
    ...
    Y = mappedX(:,ind(2:no_dims + 1));                                % throw away zero eigenvector/value
