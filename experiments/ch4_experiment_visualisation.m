...
dataset = '20newsgroup'; prefix = GLOBAL_datasparseprefix; zoom = [0, 0, 0, 0];pdiff = 0.5;mksize = 5;
%dataset = 'cifar10-cnn'; prefix = GLOBAL_dataprefix; zoom = [0, 0, 0, 0];pdiff = 2.0;mksize = 2;

export_on = 0;

%methods = {'tsne', 'largevis', 'cdr:tsne', 'cdr:largevis'};
%titles = {'t-SNE', 'LargeVis', 'CDR - t-SNE', 'CDR - LargeVis'};
methods = {'cdr:largevis'};
titles = {'CDR - LargeVis'};
...
for ii = 1:nm
    ...
    if export_on
        ...
        export_fig(sprintf('export/ch4_experiment_vis_%s_%s.pdf', dataset, methods{ii}));
    close(f2);
    end
